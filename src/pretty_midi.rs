...
mod tests {
    ...
    #[test]
    fn print_pretty_midi_debug_msg() {
        let pretty_midi = PrettyMIDI::new(480, 120.);
        let print_msg = format!("{:?}", pretty_midi);
        assert!(print_msg.contains("resolution: 480"));
        assert!(print_msg.contains("instruments: []"));
        assert!(print_msg.contains("key_signature_changes: []"));
        assert!(print_msg.contains("time_signature_changes: []"));
        assert!(print_msg.contains("lyrics: []"));
        assert!(print_msg.contains("text_events: []"));
    }
}
