...
mod tests {
    ...
    #[test]
    fn print_instrument_debug_msg() {
        let instrument = Instrument::new(0, false, "piano");
        let print_msg = format!("{:?}", instrument);
        assert!(print_msg.contains("program: 0"));
        assert!(print_msg.contains("is_drum: false"));
        assert!(print_msg.contains("name: \"piano\""));
    }
}
